...
class PreferenceModel(db: Database)(implicit ec: ExecutionContext) {
    def createPreference(preferenceData: PreferenceData): Future[Boolean] = {
      val action = (Preference += PreferenceRow(0, preferenceData.username, Option(preferenceData.gender), Option(preferenceData.year),
